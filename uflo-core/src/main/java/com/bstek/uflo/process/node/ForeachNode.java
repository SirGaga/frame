...
public class ForeachNode extends Node {
	...
	public boolean enter(Context context, ProcessInstance processInstance) {
		...
		if(StringUtils.isEmpty(variable)){
			throw new IllegalArgumentException("分支节点的var属性不能为null");
		}
		if(foreachType.equals(ForeachType.Handler)){
			...
			if(coll==null || coll.size()==0){
				throw new IllegalArgumentException("分支节点 ["+handlerBean+"] 的返回值无效!");
			}
		}else{
			if(StringUtils.isEmpty(processVariable)){
				throw new IllegalArgumentException("分支节点的流程变量属性不能为null");
			}
			...
			if(variableObj instanceof Object[]){
				...
				throw new IllegalArgumentException("分支节点监听 ["+processVariable+"] 返回值不是一个数组或集合!");
			}
		}
