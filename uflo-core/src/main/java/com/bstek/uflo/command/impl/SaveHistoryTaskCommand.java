...
public class SaveHistoryTaskCommand implements Command<HistoryTask> {
	...
	public HistoryTask execute(Context context) {
		...
		hisTask.setProcessInstanceId(processInstance.getId());
		...
		hisTask.setOperation(task.getOperation());
		//}
		hisTask.setCreateDate(task.getCreateDate());
		...
		hisTask.setEnterFlow(task.getEnterFlow());
		hisTask.setNextAssignee(task.getNextAssignee());
