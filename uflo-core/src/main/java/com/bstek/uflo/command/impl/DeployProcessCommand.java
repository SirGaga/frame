...
public class DeployProcessCommand implements Command<ProcessDefinition> {
	...
	public ProcessDefinition execute(Context context) {
		...
		if(!update && StringUtils.isNotEmpty(key)){
			...
			List<ProcessDefinition> processes=session.createCriteria(ProcessDefinition.class).add(Restrictions.eq("key",process.getKey())).addOrder(Order.desc("version")).list();
			if(processes.size()>0){
